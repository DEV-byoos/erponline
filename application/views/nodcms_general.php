...
<head>
    ...
    <?php
        ...
        if ($_SERVER["SERVER_PORT"] != "80") {
 		...
 		$mysession = _mysession('user');// modif  BYOOS_modify session helper
        ?>
    ...
    <footer>
        <div class="modal-footer">
			...
			<!--Compteur visite de Luc@s http://www.phpcs.com/  -->
				<?= lang('page_viewed'); ?>  <strong><?php echo $this->counter->visites(); ?></strong> <?= lang('times'); ?>&nbsp;&nbsp;&nbsp;&nbsp;<?= lang('connected'); ?> <strong><?php echo  $this->counter->view(); ?></strong>&nbsp;&nbsp;&nbsp;&nbsp;<?= lang('page_rendered_in'); ?> <strong>{elapsed_time}</strong> <?= lang('seconds'); ?>. <?php echo  (ENVIRONMENT === 'development') ?  'CodeIgniter Version <strong>' . CI_VERSION . '</strong>' : '' ?>
                <?=date("Y")?>
