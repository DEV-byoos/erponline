...
class Forum extends MY_Controller {
	...
	public function index($slug = false) {
		...
		if ($slug === false) {
			...
			foreach ($forums as $forum) {
				...
				$forum->count_posts  = $this->forum_model->count_forum_posts2($forum->id);
			...
			// get id from slug
			if ($forum_id = $this->forum_model->get_forum_id_from_forum_slug($slug)) {
				// create objects
				...
				// create breadcrumb
				$breadcrumb  = '<ol class="breadcrumb">';
				$breadcrumb .= '<li><a href="' . base_url('forum') . '">'.lang('home').'</a></li>';
				...
				$this->session->set_flashdata('error', ' No forum entry ');
	...
	public function create_forum() {
		...
		if(!in_array('createForum', $this->permission)) {
			redirect('/admin/', 'refresh');
		}		
		// create the data object
		$data = new stdClass();
		// if the user is not logged in as administrator, he cannot create a new forum
		if (null !==$this->session->userdata('logged_in_status')) {
		...
		$breadcrumb  = '<ol class="breadcrumb">';
		$breadcrumb .= '<li><a href="' . base_url('forum') . '">'.lang('home').'</a></li>';
		$breadcrumb .= '<li class="active">'.lang('create_a_new_forum').'</li>';
		$breadcrumb .= '</ol>';
		...
		$this->form_validation->set_rules('title', 'Forum Title', 'trim|required|alpha_numeric_spaces|min_length[4]|max_length[255]|is_unique[fo_forums.title]', array('is_unique' => ''.lang('forum_title_already_exists').''));
		...
		if ($this->form_validation->run() === false) {
			...
			if ($this->forum_model->create_forum($title, $description, $user_id)) {
				...
				$data->error = ''.lang('problem_creating_new_forum').'';
	...
	public function create_topic($forum_slug) {
		...
		$breadcrumb  = '<ol class="breadcrumb">';
		$breadcrumb .= '<li><a href="' . base_url('forum/') . '">'.lang('home').'</a></li>';
		$breadcrumb .= '<li><a href="' . base_url('forum/index/'.$forum->slug) . '">' . $forum->title . '</a></li>';
		...
		// set validation rules
		$this->form_validation->set_rules('title', 'Topic Title', 'trim|required|alpha_numeric_spaces|min_length[4]|max_length[255]|is_unique[fo_topics.title]', array('is_unique' => ''.lang('topic_title_already_exists').''));
		...
		if ($this->form_validation->run() === false) {
			...
			if ($this->forum_model->create_topic($forum_id, $title, $content, $user_id)) {
				...
				// topic creation ok
				redirect(base_url('forum/index/'.$forum_slug . '/' . strtolower(url_title($title))));
				
			} else {
				
				// topic creation failed, this should never happen
				$data->error = lang('problem_creating_topic');
	...
	public function create_post($forum_slug, $topic_slug) {
		...
		if ($this->form_validation->run() === false) {
			...
			if ($this->forum_model->create_post($topic_id, $user_id, $content)) {
				...
				// post creation ok
				redirect(base_url('forum/index/'.$forum_slug . '/' . $topic_slug));
