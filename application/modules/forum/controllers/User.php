...
class User extends MY_Controller {
	...
	public function index($username = false) {
		...
		if ($user->latest_post !== null) {
			...
			$user->latest_post->created_at = $user->username . lang('has_not_posted');
		}
		...
		if ($user->latest_topic !== null) {
			...
			$user->latest_topic->title = $user->username . lang('has_not_started_topic');
		}
		
		// create breadcrumb
		$breadcrumb  = '<ol class="breadcrumb">';
		$breadcrumb .= '<li><a href="' . base_url() . '">'.lang('home').'</a></li>';
		$breadcrumb .= '<li class="active">' . $username . '</li>';
		$breadcrumb .= '</ol>';
		
		// create a button to permit profile edition
		$edit_button = '<a href="' . base_url('user/' . $user->username . '/edit') . '" class="btn btn-xs btn-success">'.lang('edit_your_profile').'</a>';
	...
	public function register() {
		...
		$this->form_validation->set_rules('username', 'Username', 'trim|required|alpha_numeric|min_length[4]|max_length[20]|is_unique[users.username]', array('is_unique' => lang('username_already_exist')));
	...
	public function edit($username = false) {
		...
		$this->form_validation->set_rules('username', 'Username', 'trim|min_length[4]|max_length[20]|alpha_numeric|is_unique[users.username]', array('is_unique' => lang('username_already_exist')));
		$this->form_validation->set_rules('email', 'Email', 'trim|valid_email|is_unique[users.email]', array('is_unique' => lang('email_already_exist')));
			...
			if ($this->user_model->update_user($user_id, $update_data)) {
				...
				if(isset($update_data['username'])) {
					...
					if ($this->input->post('username') != '') {
						...
						$_SESSION['flash']    = lang('profile_updated');
					}
				}
				...
				if ($this->input->post('username') != '') {
					...
					$data->success = lang('profile_updated');
				...
				$data->error = lang('problem_updating_your_account');
	...
	public function delete($username = false) {
		...
		if ($_SESSION['username'] === $username) {
			...
			$breadcrumb .= '<li><a href="' . base_url() . '">'.lang('home').'</a></li>';
			$breadcrumb .= '<li><a href="' . base_url('user/' . $username) . '">' . $username . '</a></li>';
			$breadcrumb .= '<li class="active">'.lang('delete').'</li>';
			$breadcrumb .= '</ol>';
			...
			if ($this->user_model->delete_user($user_id)) {
				...
				$data->error = lang('account_not_deleted');
