...
class Nodcms_general_admin extends MY_Controller {
...
//        The new statistic
        ...
        $this->data['groups_count']=$this->NodCMS_general_admin_model->count_groups();
    ...
    function edituser($id='')
    {
		...
		$group_data = $this->NodCMS_general_admin_model->getGroupData();
	...
	*/
	public function groups()
	{
		...
		$groups_data = $this->NodCMS_general_admin_model->getGroupData();
		$this->data['groups_data'] = ($groups_data) ? $groups_data: null;

		//$this->render_template('groups/index', $this->data);
		$this->data['data_list']=$this->NodCMS_general_admin_model->get_all_group();
        $this->data['title'] = _l("Group",$this);
        $this->data['page'] = "group";
        $this->data['content']=$this->load->view($this->mainTemplate.'/groups/index',$this->data,true);
		$this->load->view($this->mainTemplate,$this->data,'');
	}	
	...
	*/
	public function create_group()
	{
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	$create = $this->NodCMS_general_admin_model->create_group($data);
        	if($create == true) {
			...
			$this->data['page'] = "group";
			$this->data['content']=$this->load->view($this->mainTemplate.'/groups/create',$this->data,true);
            $this->load->view($this->mainTemplate,$this->data,'');
        }	
	}
	...
	public function edit_group($id = null)
	{
		...
		if($id) {
			...
			if ($this->form_validation->run() == TRUE) {
	        	...
	        	$update = $this->NodCMS_general_admin_model->edit_group($data, $id);
	        	if($update == true) {
	        		...
	        		$this->session->set_flashdata('errors', 'Error occurred!!');
	        		redirect('admin/edit_group'.$id, 'refresh');
	        	}
	        }
	        else {
	            // false case
	            $group_data = $this->NodCMS_general_admin_model->getGroupData($id);
				$this->data['group_data'] = $group_data[0];
				$this->data['title'] = _l("Groups",$this);
				$this->data['page'] = "group";
				$this->data['content']=$this->load->view($this->mainTemplate.'/groups/edit',$this->data,true);
				$this->load->view($this->mainTemplate,$this->data,'');
	        }	
		}
	}
	...
	public function delete_group($id)
	{
		...
		if($id) {
			if($this->input->post('confirm')) {
				...
				$check = $this->NodCMS_general_admin_model->existInUserGroup($id);
				if($check == true) {
					$this->session->set_flashdata('error', 'Group exists in the users');
	        		redirect('admin/groups', 'refresh');
				}
				else {
					$delete = $this->NodCMS_general_admin_model->delete_group($id);
					if($delete == true) {
		        		...
		        		redirect('admin/delete_group'.$id, 'refresh');
		        	}
				}	
			}	
			else {
				...
				$this->load->view($this->mainTemplate.'admin/delete_group',$this->data,'');
			}	
		}
	}
    ...
    */
	...
	{  
		...
		$this->form_validation->set_rules('service_charge_value', 'Charge Amount', 'trim|decimal');
		$this->form_validation->set_rules('vat_charge_value', 'Vat Charge', 'trim|decimal');
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	$update = $this->NodCMS_general_admin_model->updateCompany($data, 1);
        	if($update == true) {
        		$this->session->set_flashdata('success', 'Successfully created');
        	...
        	$this->data['company_data'] = $this->NodCMS_general_admin_model->getCompanyData(1);
        	$this->data['title'] = _l("Company",$this);
			$this->data['page'] = "Company";
			$this->data['content']=$this->load->view($this->mainTemplate.'/company/index',$this->data,true);
            $this->load->view($this->mainTemplate,$this->data,'');
